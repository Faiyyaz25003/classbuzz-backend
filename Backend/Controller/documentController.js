      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg,#059669,#10b981);padding:36px 40px;text-align:center;">
            ...
            <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;">Document Approved!</h1>
            <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">Great news — your submission has been verified</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px;">
            <p style="margin:0 0 20px;color:#374151;font-size:16px;line-height:1.6;">Hello <strong style="color:#059669;">${name}</strong>,</p>
            <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.7;">We're pleased to inform you that your document has been successfully reviewed and <strong>approved</strong> by the admin team.</p>
            ...
            <p style="margin:0;color:#6b7280;font-size:14px;line-height:1.6;">You can now proceed with the next steps. If you have any questions, feel free to contact the admin office.</p>
          </td>
        </tr>
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:24px 40px;text-align:center;">
            <p style="margin:0;color:#9ca3af;font-size:13px;">Automated message from <strong style="color:#059669;">School Admin Portal</strong></p>
...
// ✅ NEW: reason parameter added
const rejectedTemplate = (name, docName, reason) => `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0"/></head>
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg,#dc2626,#ef4444);padding:36px 40px;text-align:center;">
            <span style="font-size:48px;display:block;margin-bottom:12px;">❌</span>
            <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:700;">Document Rejected</h1>
            <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">Action required — please re-upload a valid document</p>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 40px;">
            <p style="margin:0 0 20px;color:#374151;font-size:16px;line-height:1.6;">Hello <strong style="color:#dc2626;">${name}</strong>,</p>
            <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.7;">Unfortunately, your document could not be approved. Please re-upload a <strong>clear and valid</strong> version at your earliest convenience.</p>
            ...
            <!-- ✅ NEW: Admin ka custom reason — conditionally show hoga -->
            ${reason ? `
            <div style="background:#fefce8;border:1px solid #fde047;border-left:4px solid #eab308;border-radius:10px;padding:18px 20px;margin-bottom:24px;">
              <p style="margin:0 0 8px;font-size:12px;color:#713f12;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">📝 Reason from Admin</p>
              <p style="margin:0;font-size:15px;color:#422006;line-height:1.7;">${reason}</p>
            </div>
            ` : `
            <!-- Fallback generic reasons jab admin ne reason nahi diya -->
            ...
            <p style="margin:0;color:#6b7280;font-size:14px;line-height:1.6;">Please log in to your portal and re-upload the correct document. Contact the admin office if you need further assistance.</p>
          </td>
        </tr>
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:24px 40px;text-align:center;">
            <p style="margin:0;color:#9ca3af;font-size:13px;">Automated message from <strong style="color:#dc2626;">School Admin Portal</strong></p>
            <p style="margin:6px 0 0;color:#d1d5db;font-size:12px;">Please do not reply to this email.</p>
          </td>
        </tr>
      </table>
...
export const uploadDocuments = async (req, res) => {
  try {
    ...
    for (const file of req.files) {
      ...
      const newDoc = await Document.create({ user: userId, name: docName, fileUrl: file.path });
      uploadedDocs.push(newDoc);
    }
    res.status(201).json({ message: "Documents Uploaded Successfully", data: uploadedDocs });
  } catch (error) {
...
export const rejectDocument = async (req, res) => {
  try {
    ...
    // ✅ NEW: Frontend se reason lo (optional — agar blank ho toh generic reasons show honge)
    const { reason } = req.body;

    doc.status = "rejected";

    // ✅ NEW: Reason ko document mein save karo
    ...
    const emailSent = await sendEmail({
      ...
      html: rejectedTemplate(doc.user.name, doc.name, reason?.trim() || null),
    });
